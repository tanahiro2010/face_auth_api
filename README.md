...
insightfaceの検出器は画像全体を固定サイズ（既定 `640x640`, `.env` の `FACE_DET_SIZE`）に縮小してから顔を探すため、
...
> **注意: `FACE_DET_SIZE` は大きくすればよいわけではありません。** 値を上げると小さい顔は拾いやすくなりますが、
> 逆に**フレーム一杯に写った大きい顔**は検出スコアが閾値を下回り取りこぼします（実測: 顔が画面いっぱいの写真で
> det_size 640 → score 0.88 / 960 → 0.18 で 0 検出）。スマートグラス用途は「目の前の人物＝大きい顔」が主なので、
> 既定は insightface 標準の `640` とし、小さい顔はタイル分割フォールバックで補う方針です。
...
**トレードオフ**: 常に4方向で検出するため、1リクエストあたりの処理時間はおよそ4倍になります（CPU推論。det_size に比例して増加）。同時アクセスが多い環境ではサーバーのCPUリソースを多めに見積もってください。
